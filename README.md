...
我们没有让智能合约去判断「这篇研究笔记写得好不好」「这个 PR 是不是灌水」——这类判断更适合交给人。用户在 Step 4 提交贡献证明后，审核后端（`/api/attest`）通过才会签发 EIP-712 铸造授权，前端凭这份签名在 Step 5 铸造；当前审核策略是「自动通过 + 事后人工抽查」。链上合约负责的是更确定的事情：谁、在什么时间、针对哪条赛道，被授权铸造了一枚凭证。
...
* 贡献证明提交表单：成果链接校验、补充说明，提交时连接钱包并写入审核后端（`/api/proofs`）
* 铸造流程中的完整状态反馈：未连接钱包提示、错误网络下的一键切链、自动向后端换取铸造授权、铸造中/确认中的加载态、交易失败错误信息展示
